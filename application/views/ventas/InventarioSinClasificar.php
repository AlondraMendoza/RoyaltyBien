<script>
    ...
    function CargaInfo(id)
    {
        if (!$("#" + id).hasClass("cargado")) {
            ...
            $("#" + id).load("CargarInfoModeloSinClasificar", {"modelo_id": id});
            $("#" + id).addClass("cargado");
        }

    }
</script>
<h1 class="light text-shadow">INVENTARIO DE PRODUCTOS SIN CLASIFICAR</h1><br>
...
<div class="tabcontrol" data-role="tabcontrol" data-save-state="true" id='tabs'>
    ...
    <div class="frames">
        <?php foreach ($modelos->result() as $modelo): ?>
            <div class="frame" id="<?= $modelo->IdModelos ?>"></div>
        <?php endforeach; ?>
    </div>
</div>