...
class Modeloadministrador extends CI_Model {
    ...
    public function GenerarDetalleSeleccionadoQ($fechainicio, $fechafin, $ahornos, $aproducto, $amodelo, $acolor, $por, $nombre) {
        ...
        $query = $this->db->query("select p.IdProductos,cp.Nombre as producto,m.Nombre as modelo,co.Nombre as color,h.* from Productos p left join CProductos cp on cp.IdCProductos=p.CProductosId left join Modelos m on m.IdModelos=p.ModelosId left join Colores co on co.IdColores=p.ColoresId left join Hornos h on h.IdHornos=p.HornosId where  date(FechaCaptura) BETWEEN $fechainicio AND $fechafin" . $parteclasificacion . $parteproducto . $partemodelo . $partecolor . " AND convert(" . $campo . " , char(30))=" . "'" . $nombre . "'");
    ...
    public function GenerarDetalleSeleccionadoGlobal($fechainicio, $fechafin, $aclasificacion, $aproducto, $amodelo, $acolor, $por, $nombre) {
        $fechainicio = $this->FechaIngles($fechainicio);
        $fechafin = $this->FechaIngles($fechafin);
        $parteclasificacion = "";
        $parteproducto = "";
    ...
    public function GenerarReporteGlobal($fechainicio, $fechafin, $aclasificacion, $aproducto, $amodelo, $acolor) {
        $fechainicio = $this->FechaIngles($fechainicio);
        $fechafin = $this->FechaIngles($fechafin);
        $parteclasificacion = "";
        $parteproducto = "";
    ...
    public function GenerarConcentradoGlobal($fechainicio, $fechafin, $aclasificacion, $aproducto, $amodelo, $acolor, $por) {
        $fechainicio = $this->FechaIngles($fechainicio);
        $fechafin = $this->FechaIngles($fechafin);
        $parteclasificacion = "";
        $parteproducto = "";
