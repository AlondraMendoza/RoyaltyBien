...
<div class="panel primary tablasproductos" data-role="panel" style="font-size: 1.2em;<?= $oculto ?>"
    id="tabla<?= $cont ?>">
    <div class="heading">
        ...
        <div class="">
            <table class="table hovered border bordered">
                <tr class="center primary" style="font-size: 1.2em">
                    <td class="center" style="text-align: center"><span
                            onclick="CargarColores(<?= $mod . ',' . $cprod ?>)" style="font-size: 3em"
                            class="mif-undo mif-ani-hover-spanner mif-ani-slow"
                            title="Regresar a lista de Colores"></span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
                    <td class="center" style="text-align: center"><b>Clave producto:</b><br>
                        <?= $producto->IdProductos ?></td>
                    <td class="center" style="text-align: center"><b>Nombre producto:</b><br>
                        <?= $producto->NombreProducto ?></td>
                    <td class="center" style="text-align: center"><b>Modelo:</b><br> <?= $producto->NombreModelo ?>
                    </td>
                    <td class="center" style="text-align: center"><b>Color:</b><br> <?= $producto->NombreColor ?>
                    </td>
                </tr>
            </table>
            <table class="table bordered hovered border" style="font-size: 1.2em">
                <tr>
                    <td class="center" colspan="4">
                        ...
                        <button
                            id="botonclasificacion<?= $clasificacion->IdClasificaciones ?>-<?= $producto->IdProductos ?>"
                            class="botonesclasificacion button cycle-button <?= $clasificacion->Color ?>"
                            style="width: 100px;height:100px;font-size: 3em"
                            onclick="SeleccionarClasificacion(<?= $clasificacion->IdClasificaciones ?>,<?= $producto->IdProductos ?>, '<?= $clasificacion->Letra ?>')"><?= $clasificacion->Letra ?></button>
                ...
                <tr>
                    <td style="width: 45%">
                        ...
                        <div id="masdefecto1<?= $producto->IdProductos ?>" class="center">
                            <span class="mif-plus fg-darkBlue" style="font-size: 5em"
                                onclick="ApareceFormulario(1,<?= $producto->IdProductos ?>)"></span>
                        </div>
                        <div id="formulariodefecto1<?= $producto->IdProductos ?>" style="display:none">
                            ...
                            <div id="" class="input-control select full-size" style="height: 80px;">
                                <select onchange="CargarDefectos(<?= $producto->IdProductos ?>, 1)"
                                    id="catdefectos1<?= $producto->IdProductos ?>">
                                ...
                                ?>
                                    ...
                                    <option value="<?= $categoria->IdCatDefectos ?>"><?= $categoria->Nombre ?>
                                    </option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            ...
                            <input type="text" class="input-control text full-size"
                                id="claveempleadodef1<?= $producto->IdProductos ?>"
                                style="height: 80px;font-size: 1.6em"
                                onkeyup="VerificarEmpleado(1,<?= $producto->IdProductos ?>)">
                    ...
                    <td style="width: 45%">
                        ...
                        <div id="masdefecto2<?= $producto->IdProductos ?>" class="center">
                            <span class="mif-plus fg-darkGreen" style="font-size: 5em"
                                onclick="ApareceFormulario(2,<?= $producto->IdProductos ?>)"></span>
                        </div>
                        ...
                        <div id="formulariodefecto2<?= $producto->IdProductos ?>" style="display:none">
                            ...
                            <b>Categoría defectos</b><br>
                            <div id="" class="input-control select full-size" style="height: 80px;">
                                <select onchange="CargarDefectos(<?= $producto->IdProductos ?>, 2)"
                                    id="catdefectos2<?= $producto->IdProductos ?>">
                                    <option value="0">Selecciona categoría</option>
                                    <?php
                                $ci = &get_instance();
                                $ci->load->model("modeloclasificador");
                            ...
                            <b>Clave de empleado</b><br>
                            <input type="text" class="input-control text full-size"
                                id="claveempleadodef2<?= $producto->IdProductos ?>"
                                style="height: 80px;font-size: 1.6em"
                                onkeyup="VerificarEmpleado(2,<?= $producto->IdProductos ?>)">
                ...
                <tr>
                    <td colspan="2">
                        <p style="text-align: right;margin-right: 10px;">
                            <button id="botonsiguiente<?= $producto->IdProductos ?>" style="height: 80px"
                                class="button block-shadow-success text-shadow success big-button botonessiguiente"
                                onclick="Siguiente(<?= $producto->IdProductos ?>, '<?= date_format(date_create($producto->FechaCaptura), 'dmY') ?>','<?= date_format(date_create($producto->FechaCaptura), 'dmY') ?>-<?php echo str_pad($producto->IdProductos, 10, '0', STR_PAD_LEFT) ?>','<?=$producto->NombreProducto.' | '.$producto->NombreModelo.' | '.$producto->NombreColor;?>')"><span
                                    class="mif-arrow-right mif-ani-hover-horizontal"></span>
                                <?php
...
<div id="Finalizar" style="display:none">
    <div class="panel primary" data-role="panel">
        ...
        <div class="content"><br><br>
            <button id="" style="height: 80px;" class="button block-shadow-success text-shadow success big-button "
                onclick="RecargarHornos()">Continuar Clasificación</button>
            <br><br><br>
        </div>
    </div>
</div>
...
function CargarDefectos(idprod, ndef) {
    ...
    if (idcat != 0) {
        $("#divdefecto" + ndef + idprod).load("CargarDefectos", {
            "cat_id": idcat,
            "ndef": ndef,
            "idprod": idprod
        });
...
function Siguiente(idprod, fecha, codigo, descripcion) {
    ...
    if (iddefecto1 != 0 && clavepuesto1 == "") {
        Notificacion("Error", "Es necesario que captures la clave del empleado relacionado al defecto 1.", "cancel",
            "alert");
        return (0);
    }
    if (iddefecto2 != 0 && clavepuesto2 == "") {
        Notificacion("Error", "Es necesario que captures la clave del empleado relacionado al defecto 2.", "cancel",
            "alert");
        return (0);
    }
    $.post("GuardarClasificacion", {
        "idclasi": idclasi,
        "idprod": idprod,
        "defecto1": iddefecto1,
        "defecto2": iddefecto2,
        "puestodefecto1": clavepuesto1,
        "puestodefecto2": clavepuesto2,
        "fueratono": fueratono
    }, function(data) {
        if (data == "correcto") {
            Notificacion("Correcto", "La Clasificación se guardó correctamente", "check", "success");
            CodigoBarras("barcode" + idprod, codigo, "#imgbarcode" + idprod, descripcion);
            $("#imgbarcode" + idprod).printArea();
        } else {
...
function VerificarEmpleado(defecto, producto) {
    ...
    $.getJSON("VerificarEmpleado", {
        "clave": clave,
        "categoria": categoria
    }, function(data) {
        if (data.nombre == 'No se encontró trabajador') {
            $("#resclaveempleadodef" + defecto + producto).html("<b style='color:red'>" + data.nombre +
                "</b>");
        } else {
            $("#resclaveempleadodef" + defecto + producto).html("<b style='color:green'>" + data.nombre +
                "</b>");
        }
